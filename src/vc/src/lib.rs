...
impl VersionControl {
    ...
    pub fn get_changes(&self, path: &str) -> Vec<Change> {
        self.changes.get(&normalize_path(path)).cloned().unwrap_or_else(Vec::new)
    }

    pub fn get_saves(&self, path: &str) -> Vec<Save> {
        self.saves.get(&normalize_path(path)).cloned().unwrap_or_else(Vec::new)
    }
    ...
    pub fn get_sorted_changes(&self, path: &str) -> BTreeMap<String, BTreeMap<String, Vec<String>>> {
        ...
        if let Some(changes) = self.changes.get(&normalize_path(path)) {
    ...
    pub fn get_edit_history(&self, path: &str) -> BTreeMap<String, BTreeMap<String, Vec<String>>> {
        ...
        if let Some(saves) = self.saves.get(&normalize_path(path)) {
...
fn normalize_path(path: &str) -> String {
    Path::new(path).canonicalize().unwrap().to_str().unwrap().replace("\\", "/").to_string()
}
